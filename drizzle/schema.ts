...
export const receiptVouchers = mysqlTable("receiptVouchers", {
  ...
  dueDateFrom: date("dueDateFrom"),
  dueDateTo: date("dueDateTo"),
  dueDate: date("dueDate"), // للتوافق العكسي
...
// ==================== جدول سجل تدقيق OCR ====================
export const ocrAuditLogs = mysqlTable("ocr_audit_logs", {
  id: int("id").autoincrement().primaryKey(),
  branchId: int("branchId").notNull(),
  ...
  // البيانات المدخلة
  inputNetworkAmount: decimal("inputNetworkAmount", { precision: 12, scale: 2 }),
  inputDate: varchar("inputDate", { length: 10 }).notNull(), // YYYY-MM-DD
  
  // البيانات المستخرجة
  extractedAmount: decimal("extractedAmount", { precision: 12, scale: 2 }),
  extractedDate: varchar("extractedDate", { length: 10 }),
  extractedSections: json("extractedSections").$type<Array<{
    name: string;
    hostTotal: number;
  ...
  isAmountMatched: boolean("isAmountMatched").default(false),
  isDateMatched: boolean("isDateMatched").default(false),
  amountDifference: decimal("amountDifference", { precision: 12, scale: 2 }),
  
  // معلومات إضافية
  processingTimeMs: int("processingTimeMs"),
  errorMessage: text("errorMessage"),
  warnings: json("warnings").$type<Array<{
    type: string;
    severity: string;
    message: string;
    suggestion?: string;
...
export type OCRAuditLog = typeof ocrAuditLogs.$inferSelect;
export type InsertOCRAuditLog = typeof ocrAuditLogs.$inferInsert;
