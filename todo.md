...
## تطوير لوحة تحكم الأدمن في بوابة الموظفين (28 يناير 2026) - مكتمل

### صور الموظفين
- [x] إضافة API لرفع صور الموظفين (portalAdmin.uploadEmployeePhoto)
- [x] عرض صور الموظفين في بطاقات الموظفين
- [x] إمكانية رفع صورة جديدة من نافذة التفاصيل

### تعديل بيانات الموظفين
- [x] إضافة API لتعديل بيانات الموظف (portalAdmin.updateEmployee)
- [x] نموذج تعديل شامل (الاسم، الهاتف، البريد، الوظيفة، الراتب، الوثائق)
- [x] تحديث فوري للبيانات بعد الحفظ

### نظام تذكير انتهاء الوثائق
- [x] إضافة API لإرسال تذكير (portalAdmin.sendDocumentReminder)
- [x] نافذة إرسال التذكير مع خيارات متعددة (بريد/SMS/كلاهما)
- [x] زر تذكير في تبويب الوثائق

### تحسينات الأداء وتجربة المستخدم
- [x] تصميم بطاقات الموظفين الاحترافي
- [x] نافذة تفاصيل الموظف الشاملة
- [x] فلاتر الطلبات (الحالة، النوع، الفرع)
- [x] عرض الوثائق المنتهية والقريبة الانتهاء

## تطوير لوحة تحكم الأدمن في بوابة الموظفين (طلب جديد)

### 1. صور الموظفين
- [ ] إضافة حقل photoUrl في schema (موجود بالفعل)
- [ ] إنشاء API لرفع صورة الموظف
- [ ] إنشاء API لتحديث صورة الموظف
- [ ] عرض صورة الموظف في بطاقة الموظف
- [ ] عرض صورة الموظف في تفاصيل الموظف
- [ ] إضافة placeholder للموظفين بدون صور (الأحرف الأولى)

### 2. تعديل بيانات الموظفين
- [ ] إنشاء API لتحديث بيانات الموظف الكاملة
- [ ] نموذج تعديل بيانات الموظف الشخصية (الاسم، الهاتف، البريد)
- [ ] نموذج تعديل بيانات العمل (الفرع، المنصب، الراتب)
- [ ] نموذج تعديل بيانات الوثائق (الإقامة، الشهادة الصحية، العقد)
- [ ] نموذج تعديل بيانات البنك (رقم الحساب، البنك)
- [ ] تسجيل التعديلات في سجل التدقيق
- [ ] التحقق من الصلاحيات قبل التعديل

### 3. نظام تذكير انتهاء الوثائق
- [ ] إنشاء جدول لتخزين إعدادات التذكيرات
- [ ] إنشاء API لجلب الوثائق القريبة الانتهاء
- [ ] إنشاء API لإرسال تذكير بالبريد الإلكتروني
- [ ] إنشاء API لإرسال تذكير SMS
- [ ] عرض تنبيهات الوثائق المنتهية في الداشبورد
- [ ] إرسال تذكير قبل 30 يوم من الانتهاء
- [ ] إرسال تذكير قبل 7 أيام من الانتهاء
- [ ] إرسال تذكير يوم الانتهاء

### 4. تحسينات الأداء وتجربة المستخدم
- [ ] تحسين سرعة تحميل قائمة الموظفين (pagination)
- [ ] إضافة caching للبيانات المتكررة
- [ ] تحسين تصميم بطاقات الموظفين
- [ ] إضافة skeleton loading أثناء التحميل
