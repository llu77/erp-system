...
## تطوير لوحة تحكم الوثائق الشاملة (29 يناير 2026)

### 🎯 الهدف
تطوير لوحة تحكم شاملة للوثائق تعرض جميع وثائق جميع الموظفين مع الأيام المتبقية وتفاصيل شاملة للأدمن

### المتطلبات الرئيسية
- [ ] صور الموظفين - رفع وعرض صور الموظفين
- [ ] تعديل البيانات - واجهة متكاملة لتعديل جميع بيانات الموظف
- [ ] تذكير الوثائق - نظام تنبيهات عند اقتراب انتهاء صلاحية الوثائق

### المرحلة 1: تحليل النظام الحالي
- [ ] فحص جدول الموظفين الحالي
- [ ] تحديد الحقول المطلوب إضافتها
- [ ] تحديد أنواع الوثائق (إقامة، رخصة قيادة، جواز، تأمين صحي، إلخ)

### المرحلة 2: تحديث قاعدة البيانات
- [ ] إضافة حقل صورة الموظف (profileImageUrl)
- [ ] إنشاء جدول وثائق الموظفين (employeeDocuments)
- [ ] إنشاء جدول تذكيرات الوثائق (documentReminders)

### المرحلة 3: تطوير API
- [ ] API رفع صورة الموظف
- [ ] API تحديث بيانات الموظف
- [ ] API إدارة الوثائق (إضافة، تعديل، حذف)
- [ ] API جلب الوثائق المنتهية/القريبة من الانتهاء

### المرحلة 4: تطوير واجهة المستخدم
- [ ] صفحة تفاصيل الموظف مع الصورة
- [ ] نموذج تعديل بيانات الموظف
- [ ] قسم إدارة الوثائق
- [ ] عرض حالة الوثائق (صالحة، قريبة من الانتهاء، منتهية)

### المرحلة 5: نظام التذكيرات
- [ ] لوحة تحكم الوثائق المنتهية/القريبة من الانتهاء
- [ ] إشعارات للأدمن عند اقتراب انتهاء الوثائق
- [ ] تقرير شامل لحالة وثائق جميع الموظفين
...
## لوحة تحكم الوثائق الشاملة (29 يناير 2026) ✅

### 🎯 الهدف
تطوير لوحة تحكم شاملة للوثائق تعرض جميع وثائق جميع الموظفين مع الأيام المتبقية وتفاصيل شاملة للأدمن

### النتائج ✅
- [x] إحصائيات الوثائق (منتهية، قريبة من الانتهاء، نسبة الامتثال)
- [x] جدول تفصيلي للموظفين مع الأيام المتبقية لكل وثيقة
- [x] نافذة تفاصيل الموظف الشاملة (إقامة، شهادة صحية، عقد)
- [x] تقويم مرئي لانتهاء الوثائق
- [x] فلاتر حسب نوع الوثيقة والفرع
- [x] عرض صور الوثائق
- [x] 17 اختبار vitest ناجح

### الملفات المُنشأة/المُعدلة
- server/db.ts (دوال getEmployeesWithDocumentsDetailed, getDocumentsSummaryAdvanced)
- server/routers.ts (إجراءات getExpiringDocuments, getDocumentsSummary)
- client/src/pages/DocumentsDashboard.tsx (إعادة كتابة كاملة)
- server/documents-dashboard.test.ts (جديد)


## إشعارات الوثائق المنتهية + رفع صور الوثائق (29 يناير 2026)

### 🎯 الهدف
1. إشعارات تلقائية للوثائق المنتهية - إرسال بريد إلكتروني للأدمن عند اقتراب انتهاء أي وثيقة (30 يوم مسبقاً)
2. رفع وتحديث صور الوثائق - إضافة إمكانية رفع صور الوثائق مباشرة من لوحة التحكم

### المتطلبات
- [x] إنشاء دالة لفحص الوثائق المنتهية والقريبة من الانتهاء (checkAndSendDocumentExpiryReminders)
- [x] إرسال بريد إلكتروني للأدمن والمشرف العام ومشرف الفرع
- [x] جدولة الفحص يومياً (الساعة 8:00 صباحاً)
- [x] إضافة واجهة رفع صور الوثائق في نافذة تفاصيل الموظف (EmployeeDocumentsModal)
- [x] حفظ الصور في S3 وتحديث قاعدة البيانات (uploadDocumentImage API)
- [x] كتابة الاختبارات (814 اختبار ناجح)

### الملفات المُعدلة
- server/notifications/scheduledNotificationService.ts (دالة checkAndSendDocumentExpiryReminders)
- server/scheduler/cronScheduler.ts (جدولة فحص الوثائق يومياً)
- client/src/pages/DocumentsDashboard.tsx (مكون EmployeeDocumentsModal و DocumentCard)
- server/document-upload.test.ts (اختبارات رفع الوثائق)


## إصلاح لوحة الوثائق + تحسينات + صفحة التدفق المالي (29 يناير 2026)

### 🎯 الهدف
1. إظهار جميع الموظفين في لوحة الوثائق (8 موظفين بدلاً من 1)
2. تحسينات وتطويرات على الواجهة
3. مراجعة صفحة التدفق المالي

### 📋 المهام

#### المرحلة 1: إصلاح عرض الموظفين
- [x] استخدام API `getDocumentsDashboard` الموجود الذي يُرجع جميع الموظفين
- [x] تعديل DocumentsDashboard لاستخدام API الصحيح
- [x] إضافة خيار فلترة (الكل، منتهية، قريبة من الانتهاء)

#### المرحلة 2: تحسينات الواجهة
- [ ] تحسين تصميم بطاقات الموظفين
- [ ] إضافة مؤشرات بصرية أفضل لحالة الوثائق
- [ ] تحسين الإحصائيات في الأعلى
- [ ] تحسين التجاوب مع الشاشات المختلفة
#### المرحلة 3: مراجعة صفحة التدفق المالي
- [x] فحص الوظائف الحالية (الصفحة تعمل بشكل جيد)
- [x] تحديد التحسينات المطلوبة (موثقة في cash-flow-review.md)
- [ ] تنفيذ التحسينات

#### المرحلة 4: الاختبار والتحقق
- [x] اختبار عرض جميع الموظفين (8 موظفين معروضين)
- [x] تشغيل جميع الاختبارات (814 اختبار ناجح)
- [ ] اختبار الفلترة والبحث
- [ ] اختبار صفحة التدفق المالي


## تطوير شامل للوحة الوثائق (2026-01-29)

### المشكلة
- لوحة الوثائق تعرض فقط الوثائق المنتهية والقريبة من الانتهاء
- لا يوجد خيار لعرض جميع الموظفين
- المستخدم لا يستطيع متابعة جميع الموظفين

### الحل المطلوب
- [ ] إضافة تبويب "جميع الموظفين" لعرض كل الموظفين بغض النظر عن حالة الوثائق
- [ ] إضافة تبويب "سارية" لعرض الموظفين ذوي الوثائق السارية
- [ ] إضافة جدول شامل لجميع الموظفين مع كل تفاصيل الوثائق
- [ ] تحسين واجهة البحث والفلترة
- [ ] إضافة إمكانية الفرز حسب اسم الموظف/الفرع/تاريخ الانتهاء
- [ ] تحسين التجاوب مع الشاشات الصغيرة (الموبايل)
- [ ] إضافة زر تصدير قائمة الموظفين



## تطوير شامل للوحة الوثائق ✅ مكتمل (29 يناير 2026)

### المشكلة
- لوحة الوثائق كانت تعرض فقط الموظفين ذوي الوثائق المنتهية/القريبة من الانتهاء
- لم يكن هناك طريقة لمتابعة جميع الموظفين

### الحل المنفذ
- [x] إضافة تبويب "الكل" لعرض جميع الموظفين (8 موظفين)
- [x] إضافة تبويب "سارية" للموظفين ذوي الوثائق السارية
- [x] إضافة تبويب "منتهية" للوثائق المنتهية
- [x] إضافة تبويب "قريبة" للوثائق القريبة من الانتهاء
- [x] إضافة عرض شبكي (بطاقات) بالإضافة للجدول
- [x] إضافة ترتيب حسب أي عمود
- [x] تحسين التصميم والألوان
- [x] إضافة إحصائيات واضحة في البطاقات العلوية
...
## إضافة إمكانية حذف المصاريف (طلب جديد - 29 يناير 2026) ✅ مكتمل
- [x] إضافة زر حذف المصاريف للأدمن فقط (يظهر في جميع الحالات: معتمد، مدفوع، مرفوض)
- [x] إضافة نافذة تأكيد الحذف (confirm dialog)
- [x] اختبار الوظيفة (يعمل على الموبايل والديسكتوب)



## إصلاح صلاحيات مشرفي الفروع (طلب جديد - 29 يناير 2026) ✅ مكتمل
- [x] تمكين المشرفين من تعديل بيانات موظفي فروعهم (canEditEmployee)
- [x] تمكين المشرفين من رؤية موظفي فروعهم في صفحة الموظفين (filteredEmployees)
- [x] تمكين المشرفين من رؤية وثائق موظفي فروعهم في لوحة الوثائق (allEmployees filter)
- [x] التأكد من أن الحذف للأدمن فقط (canDelete = isAdmin)



## إصلاح تسجيل دخول مشرفي الفروع في بوابة الموظفين (طلب جديد - 29 يناير 2026)
- [ ] التحقق من كلمات المرور المخزنة للمشرفين (Laban, Twiq)
- [ ] إعادة تعيين كلمات المرور للمشرفين (Laban@2024, Twiq@2024)
- [ ] اختبار تسجيل الدخول في بوابة الموظفين



## إصلاح تسجيل دخول مشرفي الفروع في بوابة الموظفين (29 يناير 2026) ✅ مكتمل
- [x] التحقق من كلمات المرور المخزنة للمشرفين (وجدت المشكلة: كلمة مرور مختلفة في جدول employees)
- [x] إعادة تعيين كلمات المرور للمشرفين (laban: Laban@2024, twiq: Twiq@2024)
- [x] اختبار تسجيل الدخول (ناجح - تم التوجيه للوحة تحكم الأدمن)



## إصلاح لوحة تحكم الأدمن للمشرفين (طلب جديد - 29 يناير 2026)

### المشكلة
- المشرف عند دخوله لبوابة الموظفين يرى إجمالي الموظفين: 0
- لا يستطيع إكمال بياناته الشخصية (الهوية، تاريخ الانتهاء)
- لا يرى موظفي فرعه في لوحة تحكم الأدمن

### المهام
- [ ] تحليل تدفق البيانات في لوحة تحكم الأدمن (AdminDashboard)
- [ ] إصلاح API لجلب موظفي الفرع للمشرف
- [ ] إضافة صفحة/نافذة لتعديل البيانات الشخصية للمشرف
- [ ] إصلاح الإحصائيات لتعرض بيانات الفرع فقط للمشرف
- [ ] اختبار الوظائف



## إصلاح لوحة تحكم الأدمن للمشرفين (29 يناير 2026) ✅ مكتمل
### المشكلة الجذرية
- API `checkPortalAdminAccess` كان يبحث فقط في جدول `users`
- المشرف يسجل دخول من جدول `employees` (لأنه موظف أيضاً)
- الـ id المرسل لا يتطابق مع جدول users → النتيجة: لا بيانات

### الحل المنفذ
- [x] تعديل `checkPortalAdminAccess` ليبحث في جدولين (users و employees)
- [x] تعديل API `getEmployees` ليفلتر حسب فرع المشرف
- [x] تعديل API `getDashboardStats` ليعرض إحصائيات فرع المشرف فقط
- [x] تعديل API `getRequests` ليفلتر حسب فرع المشرف
- [x] المشرف يرى الآن 4 موظفين من فرعه فقط
- [x] الإحصائيات تعرض بيانات الفرع (موظفين: 4، طلبات موافق عليها: 13، وثائق منتهية: 2)
- [x] جميع الاختبارات ناجحة (814 اختبار)
